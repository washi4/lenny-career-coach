...
const EXTRACT_PROMPT = `You are a resume parser. Extract structured profile information from the given resume text.
  ...
  try {
    ...
    const session = await copilot.createSession({
      ...
      systemMessage: {
        mode: 'customize',
        sections: {
          identity: { action: 'replace', content: EXTRACT_PROMPT },
          tone: { action: 'remove' },
