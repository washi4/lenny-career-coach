# Job Matcher Skill Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Build an OpenCode skill that uses OpenCLI's Boss直聘 adapter to search jobs, then uses a two-stage LLM pipeline to match them against the user's resume, presenting a ranked recommendation list with one-click confirmation to send greetings.

**Architecture:** The skill is a single SKILL.md file that instructs the LLM agent how to: (1) parse a resume into a structured profile, (2) call OpenCLI boss commands via Bash to fetch job listings, (3) apply rule-based quick filtering, (4) use LLM deep evaluation for remaining candidates, and (5) present results and handle user-confirmed greetings. No new code files — the skill orchestrates existing tools (Bash for OpenCLI, LLM for analysis).

**Tech Stack:** OpenCode skill (SKILL.md), OpenCLI (`@jackwener/opencli`), Bash tool for CLI execution

**Important Context:**
- OpenCLI `boss search` returns: name, salary, company, area, experience, degree, skills, boss, security_id, url
- OpenCLI `boss detail <security_id>` returns: full JD (description, skills, welfare, company info, boss info, scale, stage, address)
- OpenCLI `boss greet` and `boss send` are **recruiter-side** — NOT for job seekers. Job seekers cannot programmatically "打招呼" via these commands.
- For job seekers, the value is **finding and filtering jobs** — the user then manually opens the URL and clicks "沟通" in Boss直聘 app/web.
- All OpenCLI boss commands require: Chrome running + logged into zhipin.com + Browser Bridge extension installed.

---

## Task 1: Create job-matcher skill directory

**Files:**
- Create: `.agents/skills/job-matcher/SKILL.md`

**Step 1: Create the skill directory**

```bash
mkdir -p .agents/skills/job-matcher
```

**Step 2: Write SKILL.md with frontmatter and overview**

Create `.agents/skills/job-matcher/SKILL.md` with the following content:
...
```bash
cat .agents/skills/job-matcher/SKILL.md | head -5
```

Expected: Shows the frontmatter with `name: job-matcher`.

**Step 4: Commit**

```bash
git add .agents/skills/job-matcher/SKILL.md
git commit -m "feat: add job-matcher skill for Boss直聘 job matching via OpenCLI"
```

---

## Task 2: Validate skill loads correctly

**Files:**
- Read: `.agents/skills/job-matcher/SKILL.md`

**Step 1: Check skill appears in skill list**

The skill should appear when listing available skills. In OpenCode, invoke `skill(name="job-matcher")` — it should load and display the SKILL.md content.

**Step 2: Verify all OpenCLI command examples are correct**

Run each command (without actual execution) to verify syntax:
```bash
opencli boss search --help
opencli boss detail --help
```

Expected: Help text shows the args match what SKILL.md documents.

**Step 3: Test a real search (if OpenCLI is installed)**
...
If OpenCLI isn't installed, skip this step — the skill itself handles setup guidance.

---

## Task 3: Integration plan for web app (future, not implemented now)

This task is documentation only — planning how to integrate job-matcher into lenny-career-coach web app later.

**Files:**
- Append to: `docs/plans/2026-03-30-job-matcher-skill.md`

**Future web integration notes:**

The web app integration would require:

1. **New tab**: Add "Job Matcher" (职位匹配) as 4th tab in `career-data.ts`
2. **Backend**: A new search endpoint that calls OpenCLI as a subprocess (similar to how `search.py` works for ChromaDB)
3. **Auth challenge**: OpenCLI requires Chrome Browser Bridge — not feasible for a deployed web app. Would need to switch to:
   - `mcp-bosszp` MCP server (supports standalone deployment with QR code login)
   - Or a custom Boss直聘 API client with cookie-based auth
4. **Resume upload**: Already supported via existing PDF upload in `InputArea.tsx`
5. **UI**: Job recommendation cards with match scores, expandable JD details, and "Open in Boss直聘" buttons

This is deferred — the OpenCode skill is the MVP.
...
|------|-------------|--------|
| 1 | Create SKILL.md with full job-matcher workflow | 15 min |
| 2 | Validate skill loads and OpenCLI commands work | 5 min |
| 3 | Document future web integration plan | 5 min |

**Total estimated effort**: ~25 minutes

**Key design decisions:**
- Skill-only approach (no new code files) — the LLM orchestrates OpenCLI via Bash
- Two-stage filtering: rules first (fast, free) → LLM second (slow, smart)
- No auto-greet — `boss greet/send` are recruiter-side commands, not job-seeker-side
- Multiple search angles to maximize coverage
- Rate-limit-aware batching for detail fetches
